...
⚡ **KRW Momentum Radar v3.0.6**는 다국가 주식 시장의 모멘텀을 실시간으로 분석하고 시각화하는 Streamlit 웹 애플리케이션입니다. 이제 단순한 모니터링 도구를 넘어서 **진정한 의미의 전체 시장 탐색 엔진**으로 진화했습니다.
...
- **변동성 가속도**: 급격한 변동성 증가를 감지하여 수직 폭등 종목 제거
- **거래 적합성 필터**: 부적합한 거래 패턴을 가진 종목 자동 실격 (자세한 내용은 .cursorrules 참조)
