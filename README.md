...
⚡ **KRW Momentum Radar v3.0.4**는 다국가 주식 시장의 모멘텀을 실시간으로 분석하고 시각화하는 Streamlit 웹 애플리케이션입니다. 이제 단순한 모니터링 도구를 넘어서 **진정한 의미의 전체 시장 탐색 엔진**으로 진화했습니다.
...
- **2가지 FMS 전략**: Standard(기본) / Stable Growth(안정 성장형)
- 1일/5일 가속도 변화량 추적
- 상위 N개 종목의 모멘텀 랭킹
...
#### Stable Growth 전략 (안정 성장형)

```
FMS = 0.4×Z(1M수익률) + 0.3×Z(3M수익률) + 0.2×Z(EMA50상대위치)
      - 0.4×Z(20일변동성) - 0.4×Z(변동성 가속도)
```

### 주요 지표

- **R_1M**: 1개월 수익률
- **R_3M**: 3개월 수익률 (Stable Growth 전략)
- **AboveEMA50**: EMA50 대비 현재가 위치
- **Breakout120**: 120일 최고가 대비 위치 (Standard 전략)
- **Slope30**: 30일 로그 기울기 (연율화, Standard 전략)
- **Vol20**: 20일 변동성 (연율화)
- **VolAcceleration**: 변동성 가속도 (Stable Growth 전략)
