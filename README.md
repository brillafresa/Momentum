...
⚡ **KRW Momentum Radar v4.0.0**는 다국가 주식 시장의 모멘텀을 실시간으로 분석하고 시각화하는 Streamlit 웹 애플리케이션입니다. 이제 단순한 모니터링 도구를 넘어서 **진정한 의미의 전체 시장 탐색 엔진**으로 진화했습니다.
...
- 세션 JSON (`fms_calibration_sessions/cal_fms_YYYYMMDD_HHMMSS.json`)과  
  가격 스냅샷 (`fms_calibration_snapshots/fms_YYYYMMDD_HHMMSS/prices_krw.pkl`)을 읽어,
- 각 종목에 대해 다음 컬럼을 가진 `fms_recalib_features.csv`를 생성합니다:
  - `R_1M, R_3M, R_6M, R2_3M, AboveEMA50, Vol20_Ann, MaxDD_Pct, rank`
  - `rank`: 1단계에서 만든 정답 순서

#### 2-2. FMS 후보 수식들의 기본 설명력 비교 (역전 비율)

```bash
python fms_recalib_evaluate_formulas.py
```

- 각 FMS 후보(`old_linear`, `linear_v1`, `nonlinear_v1` 등)에 대해:
