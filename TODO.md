...
- [x] `naive_kelly` / 표 컬럼 / 정렬 옵션 / `test_naive_kelly.py` 삭제 (FMS·기타 컬럼 불변)

### 2026-09-22 — 정렬 기본값 FMS 복원 (v5.0.9)

- [x] 사이드바 정렬: 기본 `FMS(현재)` · 옵션 2번째 `나이브 켈리(20D)`
- [x] `momentum_now_and_delta` 기본 정렬 FMS desc 복원
...
- [x] 원인: `harmonize_calendar`가 유니온 2y 길이 대비 coverage → LBRX/VIA(~11m) 탈락
- [x] coverage 분모 = 컬럼 `[first_valid, last_valid]` (leading/trailing NaN 제외)
- [x] pytest: `test_native_asof_calendar` · `test_batch_ui_fms_paths` · 전체 스위트
- [x] work-plan: `docs/work-plans/2026-08-13-ipo-native-span-coverage.md`
...
- [x] 원인: cold 심볼까지 5d probe → Yahoo 호출 폭증 → USA 선행 청크 실패
- [x] warm-only probe; cold는 full download만
- [x] LIVE: `harness.smoke_multi_market_batch` · `harness.smoke_usa_first_batch`
- [x] 로컬 운영형 FREE 풀 배치 + UI 신규탐색 USA 확인
- [x] work-plan: `docs/work-plans/2026-08-08-cache-probe-warm-only.md`
