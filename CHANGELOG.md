...
- **R² 기반 급등주 필터링**: FMS 공식에 3개월 로그 수익률의 결정계수(R²) 지표 추가
  - `r_squared_3m()` 함수 구현: `scipy.stats.linregress`를 사용한 R² 계산
  - 높은 R²: 안정적인 우상향 추세 (매끄러운 상승 곡선)
  - 낮은 R²: 횡보 후 급등, 계단식 급등 등 불안정한 패턴 (감점 처리)
- **scipy 의존성 추가**: R² 계산을 위한 `scipy>=1.10.0` 추가

### 변경

- **FMS 공식 전면 수정**: 급등주 필터링 강화를 위한 가중치 재조정
  - 기존: `FMS = 0.4×Z(1M수익률) + 0.3×Z(3M수익률) + 0.2×Z(EMA50상대위치) - 0.4×Z(20일변동성)`
  - 신규: `FMS = 0.2×Z(1M수익률) + 0.3×Z(3M수익률) + 0.3×Z(3M_R²) + 0.2×Z(EMA50상대위치) - 0.2×Z(20일변동성)`
  - 1M 수익률 가중치: 0.4 → 0.2 (감소)
  - 3M R² 가중치: 0.0 → 0.3 (신규 추가)
  - 변동성 페널티: -0.4 → -0.2 (완화)
- **R2_3M 컬럼 추가**: 모멘텀 테이블에 R² 값 표시 (소수점 3자리)

### 개선

- **급등주 필터링 정확도 향상**: R² 지표로 횡보 후 급등/계단식 급등 패턴 자동 감지
- **안정적 추세 종목 발굴**: 높은 R²를 가진 종목 우선 표시로 리스크 감소
- **문서 업데이트**: README.md, .cursorrules에 새로운 FMS 공식 및 R² 설명 추가
...
- **배치 스캔 결과 메시지 UI 개선**: + 버튼 클릭 시 메시지가 넓은 폭에 표시되도록 개선
  - 기존: 메시지가 버튼 컬럼(1/4 폭)에 표시되어 줄바꿈 발생
  - 변경: 메시지를 페이징 컨트롤 위 넓은 영역에 표시하여 가독성 향상
- **데이터 coverage 임계값 완화**: harmonize_calendar 함수의 coverage 임계값을 0.9에서 0.5로 완화
  - 신규 상장 종목이나 데이터가 일부 없는 종목도 포함 가능
  - 누락된 종목에 대한 경고 메시지 추가
- **당일 고가/저가 0 처리**: 국장 개장 시간(9시~9시 20분) Yahoo Finance 데이터 이슈 대응
  - 당일 고가/저가가 0인 경우 전일 고가/저가로 대체하여 True Range 계산
  - 조건: 당일 고가=0 AND 당일 저가=0 AND 전일 종가>0 AND 전일 고가>0 AND 전일 저가>0
  - 디버그 정보에 원본 값과 수정 여부 표시

### 개선

- **코드 정리**: 사용하지 않는 변수(`has_message`) 제거
- **로깅 개선**: harmonize_calendar에서 제외된 종목 및 누락된 관심종목 로깅 추가
...
- **거래 적합성 필터 디버그 로깅 확대**: 모든 국가 종목(-999 발생 종목)에 대해 디버그 정보 표시
  - 기존: 한국 종목만 제한적으로 표시
  - 변경: 미국, 한국, 일본 모든 국가 종목 표시
  - 국가 정보 표시로 종목 구분 용이
- **디버그 로그 UI 개선**: 실격된 종목 정보를 더 읽기 쉽게 표시
  - 요약 테이블: 실격된 모든 종목을 한눈에 확인 (종목, 국가, 필터 상태, OHLC 데이터, 30% 초과 일수, 최근 변동률 등)
  - 상세 정보: 각 종목별 expander로 접기 가능한 상세 디버그 정보
    - 기본 정보 (OHLC 데이터 유무, 데이터 포인트, 마지막 날짜)
    - 최근 데이터 상세 (전일/당일 종가, 고가, 저가, True Range 변동률)
    - True Range 구성 요소 상세 (고가-저가, 고가-전일종가 갭, 저가-전일종가 갭)
    - 30% 초과 날짜별 상세 정보 (날짜, 변동률, 종가, 전일종가, 고가, 저가) 표 형식
    - 하방리스크 상세 정보 (-7% 미만 날짜별 정보) 표 형식
...
- **거래 적합성 필터 디버그 로깅**: 오전 장 시작 시간 국내 종목 -999 문제 진단을 위한 상세 디버그 정보 제공
  - OHLC 데이터 다운로드 상태 정보 (성공/실패, 누락된 종목, 데이터 범위)
  - 실격된 한국 종목별 상세 디버그 정보
    - OHLC 데이터 유무 및 데이터 포인트 수
    - 최근 데이터 상세 (전일/당일 종가, 고가, 저가, True Range 변동률)
    - True Range 구성 요소 상세 (고가-저가, 고가-전일종가 갭, 저가-전일종가 갭)
    - 30% 초과 날짜별 상세 정보 (날짜, 변동률, 종가, 전일종가, 고가, 저가)
    - 하방리스크 상세 정보 (-7% 미만 날짜별 정보)
  - 현재 시간(KST) 정보로 문제 발생 시각 확인 가능
  - 디버그 로그 영역에서 JSON 형태로 복사 가능

### 개선

- **문제 진단 능력 향상**: 오전 장 시작 시간 데이터 수집 문제를 상세 로그로 진단 가능

## [3.7.1] - 2025-11-19

### 수정

- **레버리지/인버스 ETF 필터링 강화**: 배치 스캔 시 레버리지/인버스 ETF 탐색 제외 기능 개선
  - 알려진 레버리지/인버스 ETF 티커 패턴 자동 감지 (LLYX, SMST, GGLL, GOOX 등)
  - 숫자+X 패턴 (2X, 3X 등) 자동 감지
  - 레버리지/인버스 키워드 자동 감지 (Leverage, Inverse, Short, Bear, Ultra 등)
- **관심종목 추가 실패 시 피드백 개선**: 배치 결과에서 관심종목 추가 시 실패 이유 명확히 표시
- **수동 추가 기능 개선**: 수동 관리에서 어떤 종목이든 추가 가능하도록 제한 제거 (레버리지/인버스 ETF 포함)

### 개선

- **코드 정리**: 사용하지 않는 변수 제거 및 import 최적화

## [3.7.0] - 2025-11-14

### 추가

- **동적 데이터 기간 계산**: 사용자가 선택한 옵션(rv_window, tail_days)에 따라 필요한 최소 데이터 기간만 동적으로 계산하여 다운로드
- **차트 기간과 데이터 다운로드 기간 분리**: 차트 표시 기간과 계산에 필요한 데이터 기간을 분리하여 효율성 향상

### 수정

- **차트 기간 1M 선택 시 버그 수정**: 한국 상장 종목의 Filter_status "데이터 기간 부족" 문제 해결
- **세부 보기 차트 기간 수정**: 선택된 차트 기간에 맞춰 차트 표시 (기존 2Y 고정 문제 해결)
- **R_6M 값 None 문제 수정**: 충분한 데이터를 다운로드하여 정상 계산 보장
- **FMS -999 문제 수정**: 거래 적합성 필터를 위한 충분한 데이터 다운로드 보장

### 개선

- **비교 차트 로그 스케일 고정**: 항상 로그 스케일로 표시 (체크박스 제거)
- **UI 개선**: 수익률-변동성 이동맵 블록의 불필요한 표시 제거 및 레이아웃 정리
- **설명문 제거**: 수익률-변동성 이동맵 하단의 설명문 제거로 UI 간소화
- **사이드바 설정 통합**: rv_window와 tail_days를 사이드바로 이동하여 설정 통합

### 변경

- **데이터 다운로드 최적화**: 사용자가 선택한 옵션에 따라 필요한 최소 기간만 다운로드하여 효율성 향상
- **사용하지 않는 코드 제거**: chart_period_map 등 미사용 변수 제거

## [3.6.3] - 2025-11-13

### 변경

- **차트 기간 옵션 개선**: 1M 옵션 추가, 5Y 옵션 제거 (1M, 3M, 6M, 1Y, 2Y)
- **표시 종목 수 옵션 확장**: 10, 20, 30, 40, 50, 60으로 확장 (기본값 20 유지)
- **버전 일관성 개선**: UI 타이틀과 코드 버전 불일치 해결
- **버전 관리 가이드 추가**: `.cursorrules`에 버전 변경 시 업데이트해야 할 파일 목록 명시
